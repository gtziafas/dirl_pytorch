...
def load_dataset(name: str, datasets_directory: str = './resources') -> Tuple[List[Tensor], ...]:
    ...
    ds = pkl.load(open(os.path.join(datasets_directory, file_map[name]), 'rb'))
    return [s.float() for s in ds] 

    
# def load_dataset(name: str, datasets_directory: str = './resources_Bk') -> Tuple[List[Tensor], ...]:
